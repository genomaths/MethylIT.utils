...
gammaMixtCut <- function(LR, post.cut = 0.5, div.col = NULL, tv.col = NULL, 
    tv.cut = 0.25, find.cut = FALSE, control.names = NULL, treatment.names = NULL, 
    column = c(hdiv = FALSE, TV = FALSE, wprob = FALSE, pos = FALSE), 
    classifier = c("logistic", "pca.logistic", "lda", "svm", "qda", 
        "pca.lda", "pca.qda"), prop = 0.6, clas.perf = FALSE, cut.interval = c(0.5, 
        0.8), cut.incr = 0.01, stat = 1, maximize = TRUE, num.cores = 1L, 
    tasks = 0L, tol = .Machine$double.eps^0.5, maxiter = 1000, ...) {
    ...
    # To remove divs == 0. The methylation signal only is given for
    # divs > 0
    divs <- divs[div > 0]
    ...
    zerofun <- function(x) {
        dgamma(x, shape = par1[1], scale = par1[2]) - dgamma(x, shape = par2[1], 
            scale = par2[2])
    }
    
    zero <- try(uniroot(zerofun, interval = c(lower, upper), tol = .Machine$double.eps^0.5, 
        maxiter = 1000), silent = TRUE)
    if (!inherits(zero, "try-error")) 
        zero <- unlist(zero) else zero <- rep(NA, 5)
    ...
    if (find.cut) {
        ...
        while (k < length(cuts) && !opt) {
            ...
            conf.mat <- evaluateDIMPclass(dmps, column = column, control.names = control.names, 
                treatment.names = treatment.names, classifier = classifier, 
                prop = prop, output = "conf.mat", num.cores = num.cores, 
        ...
        conf.mat <- c(Cutpoint = cutFun(cuts[k]), PostProbCut = cuts[k], 
            conf.mat)
    }
    # --------------------------------------------------------------------
    # #
    ...
    y <- structure(list(gammaPars = y1$gamma.pars, lambda = y1$lambda, 
        loglik = y1$loglik), class = "GammaMixt")
    rm(y1)
    gc()
    if (clas.perf && !find.cut) {
    ...
    if (find.cut) {
        ...
        cat("\n")
        cat("Cytosine sites with treatment PostProbCut >=", cuts[k], 
            "have a \n")
