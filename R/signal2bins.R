...
signal2bins <- function(signal, regions, stat = "mean", nbins = 20L, 
    ...
    absolute = FALSE, na.rm = TRUE, missings = 0, region.size = 200, 
    ...
    if (!is.null(streamUp)) {
        ...
        upr <- GeneUpDownStream(GR = regions, upstream = streamUp, 
            onlyUP = TRUE)
        upr <- binBuilder(regions = upr, num.bins = nbinsUP, num.cores = num.cores, 
            verbose = verbose)
    ...
    if (!is.null(streamDown)) {
        ...
        dwr <- GeneUpDownStream(GR = regions, downstream = streamDown, 
            onlyDown = TRUE)
        dwr <- binBuilder(regions = dwr, num.bins = nbinsDown, num.cores = num.cores, 
            verbose = verbose)
        if (verbose) 
